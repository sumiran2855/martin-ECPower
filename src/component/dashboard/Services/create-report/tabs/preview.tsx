    ...
    >
        ...
        <div className="space-y-4 mx-6">
          <div>
            ...
            <div className="flex justify-between my-2">
              ...
              <span>{data.creatingDate?.creationDate || "-"}</span>
            </div>
            <div className="flex justify-between my-2">
              <span className="font-medium">Service type:</span>
              <span>{data.creatingDate?.serviceType || "-"}</span>
            </div>
            <div className="flex justify-between my-2">
              <span className="font-medium">Description:</span>
              <span>{data.creatingDate?.serviceDescription || "-"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 mx-6">
          <div>
            ...
            <div className="flex justify-between my-2">
              ...
              <span>{data.creatingDate?.deliveryDate || "-"}</span>
            </div>
            <div className="flex justify-between my-2">
              <span className="font-medium">Service technician:</span>
              <span>{data.resources[0]?.serviceTechnician || "-"}</span>
            </div>
            <div className="flex justify-between my-2">
              <span className="font-medium">Work type:</span>
              <span>{data.resources?.workType || "-"}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody>
              {data.itemUsages && data.itemUsages.length > 0 ? (
                data.itemUsages.map((item: any, index: number) => (
                  <tr key={index} className={`${darkMode ? "border-gray-700" : "border-gray-200"} border-b`}>
                    <td className="py-4 px-6">{item.partNumber || "-"}</td>
                ...
                <tr>
                  <td className="py-4 px-6 text-gray-500" colSpan={5}>No items added</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {data.resources ? (
                ...
                <tr key={index} className={`${darkMode ? "border-gray-700" : "border-gray-200"} border-b`}>
                  <td className="py-4 px-6">{resource.deliveryCreationDate || "-"}</td>
                  <td className="py-4 px-6">{resource.serviceTechnician || "-"}</td>
                  <td className="py-4 px-6">{resource.workType || "-"}</td>
                  <td className="py-4 px-6">{resource.resourceQuantity || "-"}</td>
                  <td className="py-4 px-6">{resource.unit}</td>
                </tr> ))
              ) : (
                <tr>
                  <td className="py-4 px-6 text-gray-500" colSpan={5}>No resources added</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          className={`${
            darkMode
              ? "text-gray-100 bg-blue-800 hover:bg-blue-700"
              : "text-gray-100 bg-blue-900 hover:bg-blue-800"
          } border border-gray-300 rounded-md px-6 py-2 font-medium cursor-pointer`}
        >
